...
class SequenceGenerator {
    constructor(options = {}) {
        ...
        this.requestRender = typeof options.requestRender === 'function'
            ...
            : () => {};
        this.onSequenceChange = typeof options.onSequenceChange === 'function'
            ? options.onSequenceChange
            : () => {};
        this.onPlay = typeof options.onPlay === 'function'
            ? options.onPlay
            : () => {};
        ...
        this.onStatus = typeof options.onStatus === 'function'
            ? options.onStatus
            : () => {};
        this.onEditorVisibilityChange = typeof options.onEditorVisibilityChange
            === 'function'
            ? options.onEditorVisibilityChange
            : () => {};
        this.getCurrentPose = typeof options.getCurrentPose === 'function'
            ? options.getCurrentPose
            : null;
        this.isOrientationConstrained = typeof options.isOrientationConstrained
            === 'function'
            ? options.isOrientationConstrained
            : () => true;
        this.getCurrentChainPose = (
            typeof options.getCurrentChainPose === 'function'
            || typeof options.getCurrentJointState === 'function'
        )
            ? (
                options.getCurrentChainPose
                ?? options.getCurrentJointState
            )
            : () => this.sequencePlayer?.getJointState?.() ?? [];
        this.setCurrentChainPose = (
            typeof options.setCurrentChainPose === 'function'
            || typeof options.setCurrentJointState === 'function'
        )
            ? (
                options.setCurrentChainPose
                ?? options.setCurrentJointState
            )
            : (joints) => this.sequencePlayer?.setJointState?.(joints);
        this.sampleChainPose = typeof options.sampleChainPose === 'function'
            ? options.sampleChainPose
            : null;
        ...
        this.recordPositionTolerance = Number.isFinite(
            options.recordPositionTolerance,
        )
            ? Math.max(0, options.recordPositionTolerance)
            : 0.012;
        this.recordRotationTolerance = Number.isFinite(
            options.recordRotationTolerance,
        )
            ? Math.max(0, options.recordRotationTolerance)
            : 0.12;
        this.recordSolveTimeoutMs = Number.isFinite(
            options.recordSolveTimeoutMs,
        )
            ? Math.max(100, options.recordSolveTimeoutMs)
            : 5000;
        ...
        this.params = {
            editorOpen: options.editorOpen !== false,
            visualizationVisible: options.visualizationVisible !== false,
            trajectoryVisible: options.trajectoryVisible === true,
        };
    ...
    addResolverKeyframe({
        ...
        const keyframe = {
            id: typeof id === 'string' && id ? id : this._nextId(),
            label,
            kind: 'resolver',
            resolver,
            ...
            toleranceProfile: typeof toleranceProfile === 'string'
                ? toleranceProfile
                : undefined,
        };
    ...
    async _waitForSolverToSettle() {
        ...
        const initialRevision = Number.isFinite(player?.getSolveRevision?.())
            ? player.getSolveRevision()
            : null;
        player?.queueSolveFromTarget?.();
        if (typeof player?.hasPendingSolve !== 'function') {
            return true;
        }
    ...
    resolveJointState(targetSpec, context, outJointState) {
        const keyframe = this._findTargetKeyframe(targetSpec);
        const chainPose = this._sanitizeChainPose(keyframe?.chainPose);
        if (!chainPose || !Array.isArray(outJointState)) {
            return false;
        }
    ...
    setPlaybackActive(active) {
        this._playbackActive = active === true;
    ...
    setEditorOpen(open) {
        this.params.editorOpen = open === true;
        if (this._panel) {
    ...
    setVisualizationVisible(visible) {
        this.params.visualizationVisible = visible === true;
        if (this._transformControls) {
    ...
    setTrajectoryVisible(visible) {
        this.params.trajectoryVisible = visible === true;
    ...
    _findTargetKeyframe(targetSpec) {
        if (
            ...
            || typeof targetSpec.keyframeId !== 'string'
        ) {
            return null;
        }
    ...
    _resolveKeyframePose(
        ...
        if (keyframe.kind === 'resolver') {
            if (typeof keyframe.resolver !== 'function') {
                return false;
            }
    ...
    _createSceneControls() {
        ...
        this._transformControls.addEventListener(
            'dragging-changed',
            (event) => {
                this._isDraggingTarget = event.value === true;
    ...
    _collectGraphFrames() {
        ...
        this.keyframes.forEach((keyframe, index) => {
            const pose = this._getKeyframeChannelPose(keyframe);
            if (!pose || !Array.isArray(pose[activeGroup])) {
                return;
            }
    ...
    _drawGraphGrid(
        ...
        channels.forEach((channel, index) => {
            ...
            const pose = keyframe
                ...
                : null;
            if (pose && Array.isArray(pose[channel.group])) {
